...
function fButton_Callback(hObject, eventdata, handles)
    ...
    if inputTypeVal == 1 % polynomial value input
        [k, num, den, c] = getValuesFromInput('F(s)');
    else %Pole zero value input
        [k, num, den, c] = poleZeroInput('F(s)');
    end
    if c ~= 1   
        setGlobal_f(k, num, den);
        changeText(handles);
    end
...
function cButton_Callback(hObject, eventdata, handles)
    inputTypeVal = getGlobal_inputType();
    if inputTypeVal == 1 % polynomial value input
        [k, num, den, c] = getValuesFromInput('C(s)');
    else %Pole zero value input
        [k, num, den, c] = poleZeroInput('C(s)');
    end
    if c ~= 1
        setGlobal_c(k, num, den);
        changeText(handles);
    end
...
function gButton_Callback(hObject, eventdata, handles)
    ...
    if inputTypeVal == 1 % Polynomial input type
        [k, num, den, c] = getValuesFromInput('G(s)');
    else % Pole/zero input type
        [k, num, den, c] = poleZeroInput('G(s)');
    end
    if c ~= 1
            setGlobal_g(k, num, den);
            changeText(handles);
    end
...
function hButton_Callback(hObject, eventdata, handles)
    ...
    if inputTypeVal == 1   % Polynomial input value
        [k, num, den, c] = getValuesFromInput('H(s)');
    else    % Pole/Zero input type
        [k, num, den, c] = poleZeroInput('H(s)');
    end
    if c ~= 1
        setGlobal_h(k ,num, den);
        changeText(handles);
    end
...
function changeText(handles)
    ...
    cla
    axis off;
    f_sys= tf(fK.*fNum, fDen);
    F = symbolicTF(f_sys, s);
    F = latex(F);
    F = ['$',F,'$'];
